...
// test user info
describe("user info", () => {
  test("should give user info", async () => {
    const req = {};
    const res = {
      json: jest.fn(),
    };
  ...
  test("should handle error", async () => {
    ...
    await expect(userInfo(req, res)).rejects.toThrow("DB error");
  });
});

describe("getschool list", () => {
  test("should give school list", async () => {
    const req = {};
    ...
    await getSchoolList(req, res);
    expect(School.find).toHaveBeenCalledWith(req);
    expect(res.json).toHaveBeenCalledWith({
      data: {
        _id: "1",
        name: "test school",
  ...
  test("should handle error", async () => {
    const req = {};
    School.find.mockRejectedValue(new Error("DB error"));
    const res = {
      json: jest.fn(),
    };
    await expect(getSchoolList(req, res)).rejects.toThrow("DB error");
  });
});
